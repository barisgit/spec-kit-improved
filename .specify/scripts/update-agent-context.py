...
@app.command()
def main(
    assistant: str = typer.Argument(help="AI assistant type (claude)"),
    ...
    try:
        ...
        if assistant != "claude":
            if json_output:
                ...
                echo_error(f"This script is configured for claude, got: {assistant}")
        ...
        try:
            assistant_provider = get_assistant("claude")
            ...
            error_msg = f"Failed to get context file for claude: {e}"
            if json_output:
        ...
        # Update context file
        success = update_context_file(context_file, context_info, "claude")
