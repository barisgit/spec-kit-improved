...
class TestAIAwareTemplateRendering:
    ...
    def test_claude_specific_template_rendering(
        ...
        assert "Claude Integration:" in content
        assert "GitHub Actions integration with Claude Code" in content
        assert "Setup Requirements:" in content
        ...
        # Should contain Claude workflow integration (flexible matching)
        assert "Claude" in constitution_content
...
class TestInjectionPointPerformance:
    ...
    def test_template_discovery_performance(self, template_service: TemplateService):
