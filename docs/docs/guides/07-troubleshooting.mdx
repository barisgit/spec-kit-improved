...
**Error**: Commands not working in Claude Code

**Fix**:
1. Ensure project has been initialized: `specifyx init`
2. Check `.claude/commands/` directory exists
3. Regenerate commands: `specifyx refresh-templates`
4. Verify CLAUDE.md includes project context

### Agent Template Errors
**Error**: AI assistant prompts not generating correctly

**Fix**:
1. Check assistant configuration: `specifyx config show`
2. Validate assistant is supported: `specifyx add-ai --list`
3. Regenerate templates: `specifyx refresh-templates --ai your-assistant`

## Documentation Issues

### Auto-generated Docs Out of Sync
**Error**: API reference doesn't match code

**Fix**:
```bash
cd docs
pnpm generate          # One-time generation
pnpm generate:watch    # Continuous updates during development
pnpm dev              # Development server with auto-sync
```
...
2. Install dependencies: `cd docs && pnpm install`
3. Clear cache: `rm -rf docs/.docusaurus docs/build`
